...
public class SearchServiceImpl implements SearchService {
    ...
    private SearchResponse parseSearchResult(SearchResult searchResult) {
        ...
        //MetricAggregation aggregations = searchResult.getAggregations();//获取聚合结果集
        //从聚合结果集中获取分类的聚合
        TermsAggregation caregoryAgg = aggregations.getTermsAggregation("categoryId");
        List<TermsAggregation.Entry> categoryBuckets = caregoryAgg.getBuckets();

        List<String> categories = buckets.stream().map(bucket -> {
            HashMap<Object, Object> map = new HashMap<>();
            map.put("id", Long.valueOf(bucket.getKeyAsString()));
        ...
        response.setBrand(categoryVO);
        response.setCatelog(null);

        //规格参数
        ChildrenAggregation aggregation = aggregations.getChildrenAggregation("attrAgg");
        ...
        List<SearchResponseAttrVO> attrVOS =attrBuckets.stream().map(bucket ->{
            ...
            attrVO.setProductAttributeId(Long.valueOf(bucket.getKeyAsString()));//获取attrId聚合的attrName子聚合
            TermsAggregation attrNameAgg = bucket.getTermsAggregation("attrName");//获取每个attrId聚合的attrName
            ...
            List<String> values = attrValueAgg.getBuckets().stream().map(valueBucket -> valueBucket
                    .getKeyAsString()).collect(Collectors.toList());
    ...
    private String buildDslQuery(SearchParamVO searchParamVO) {
        ...
        //放查询条件
        boolQueryBuilder.must(QueryBuilders.matchQuery("name", keyword).operator(
                Operator.AND));
        //放过滤条件
        //放分类过滤
        ...
        if (props != null && props.length > 0) {
            for (String prop : props) {
                ...
                if (attrProps != null && attrProps.length == 2) {
                    boolQuery.must(QueryBuilders.termsQuery("attrValueList.attrId", attrProps
                            [0]));
        ...
        NestedAggregationBuilder nestedAggregationBuilder = AggregationBuilders.nested("attrAgg", "attrValueList")
                .subAggregation(AggregationBuilders.terms("attrId").field("attrValueList")
    ...
    public static void main(String[] args) {
        ...
        searchParamVO.setCatelog3(new String[]{"255"});
